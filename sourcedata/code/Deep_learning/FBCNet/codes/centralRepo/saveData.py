...
path = r'WBCIC_SHU Motor Imagery dataset'
...
def parseSHUFile(dataPath, epochWindow=[0, 4], chans=list(range(29))):
    fs = 250
    ...
    file_name = dataPath[-4:-1]
    filepath = os.path.join(path, 'S'+file_name + '.mat')

    dat=sio.loadmat(filepath)
    data_or=dat['data']
    labels=np.ravel(dat['labels'])-1

    x_train = data_or[:150].transpose(1, 2, 0)
    y_train = labels[:150]
    x_test = data_or[150:].transpose(1, 2, 0)
    y_test = labels[150:]

    data = {'x': x_train, 'y': y_train, 'c': ch_names, 's': fs}, \
           {'x': x_test, 'y': y_test, 'c': ch_names, 's': fs}
    return data


def parseSHUDataset(datasetPath, savePath, epochWindow=[0, 4], chans=list(range(30))):

    subjects = []
    test_subjects = []
    for i in range(1, 157):  # 733
        subjects.append('S' + str(i).zfill(3) + 'E')
        test_subjects.append('S' + str(i).zfill(3) + 'T')
    ...
    for iSubs, subs in enumerate(subAll):
        for iSub, sub in enumerate(subs):
            if not os.path.exists(os.path.join(datasetPath, sub + '.mat')):
                raise ValueError('The SHU original dataset doesn\'t exist at path: ' +
            ...
            data = parseSHUFile(sub, epochWindow=epochWindow, chans=chans)
            savemat(os.path.join(savePath, subL[iSubs] + str(iSub + 1).zfill(3) + '.mat'), data[iSubs])
...
def parseBci42aFile(dataPath, labelPath, epochWindow=[0, 4], chans=list(range(22))):
    ...
    # Epoch the data
    eventCode = [mne.events_from_annotations(raw_gdf)[1]['768']]  # Fixed 2020.12.15 11:35
