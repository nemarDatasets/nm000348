...
def ho(datasetId = None, network = None, nGPU = None, subTorun=None):
    ...
    # datasetId ->  0:BCI-IV-2a data,    1: Korea data  # 2: SHU data
    ...
    if datasetId == 1:
        ...
        config['modelArguments'] = {'nChan': 29, 'nTime': 1000, 'dropoutP': 0.5,
                                'nBands': 9, 'm': 16, 'temporalLayer': 'VarLayer',
                                'nClass': 2, 'doWeightNorm': True}
   ...
   #%% Find all the subjects to run 
    ...
    for iSub, sub in enumerate(subs):
        ...
        # idxTrain=np.arange(300,400)
        # idxTest=np.arange(400,500)

        '''修改训练测试集，取部分测试数据用于训练
            #训练集+30% 测试级-30%
            tlen=len(idxTest)
            adapt_len=np.int(tlen*0.9)
            tsd=np.array(idxTest)
            for ids in range(adapt_len):
                idxTrain.append(idxTest[ids])
            idxTest = list(tsd[adapt_len:])
        '''
