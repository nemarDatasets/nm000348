...
def cv(datasetId = None, network = None, nGPU = None, subTorun=None):
    ...
    if datasetId == 1:
        ...
        config['modelArguments'] = {'nChan': 58, 'nTime': 1000, 'dropoutP': 0.5,
                                    # 'nBands': 9, 'm': 16, 'temporalLayer': 'VarLayer',
                                    'nBands': 9, 'm': 32, 'temporalLayer': 'LogVarLayer',  # New FBCNet
                                    'nClass': 2, 'doWeightNorm': True}  # 'nClass': 4
    ...
    #import networks
    if config['network'] in networks.__dict__.keys():   #   deepConvNet eegNet  FBCNet
...
if __name__ == '__main__':
    ...
    if count >3:
        ...
        subTorun = None
    # subTorun=list(np.arange(206,251))
